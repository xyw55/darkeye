...
public class JedisTemplate<T> {
	...
	protected boolean handleJedisException(JedisException jedisException) {
		if (jedisException instanceof JedisConnectionException) {
		    logger.error(String.format("Redis some connection fails.(active:%s,idle:%s)",jedisPool.getNumActive(),jedisPool.getNumIdle()), jedisException);
	...
	*/
	...
	public Object evalSHA(final String shakey, final int cnt, final String ...params) {
        return execute(new JedisAction<Object>() {
            @Override
            public Object action(Jedis jedis) {
                return jedis.evalsha(shakey, cnt, params);
            }
        });
    }
	...
	public static void destroyJedis(Jedis jedis){
	    if(null!=jedis&&jedis.isConnected()){
	        try {
                ...
                logger.error("erroe destroyredis",e);
            }
	    }
	}
	public boolean isStatusOk(String result) {
        return "OK".equals(result)||"+OK".equals(result);
    }
}
